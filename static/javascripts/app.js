...
app.controller('AppCtrl', function($scope,$http,$location) {
  $scope.go = function(e){e.preventDefault();var href = e.target.attr('href');console.log(href);location.assign(href);}
  $scope.init = function(){
    $http.get('http://140.119.19.39:8000/islogin/').success(function(response) {
console.log(response);
      if (response[0].result == 'success') {
        $scope.member = '我的推薦';
        $('.md-toolbar-tools').append('<a  style="color:white;font-size:.75em" href="http://140.119.19.39:8000/accounts/logout">登出</a>');
      }

      else{
        $scope.member = '登入';
        $('.member').attr('href',"http://140.119.19.39:8000/login/facebook/?next=http://140.119.19.39:8000/static/index.html");
      }
    });
  ...
  $scope.departments = ['法律','資管','應數','政治','財政','資科','中文','金融','日文','英文'];
  ...
  $scope.search = function(num){
    ...
    $http.get('http://140.119.19.39:8000/dosearch/'+'?department='+$scope.tags+'&time='+$scope.tags2+'&stars='+$scope.tag3.split('')[0]+'&key='+key+'&page='+num)
      .success(function(response){
          ...
          setTimeout(function(){$('.pager'+num).addClass('active');},200);
      });
 ...
 $scope.iddd = id;
    $http.get('http://140.119.19.39:8000/islogin').success(function(response) {
      if (response[0].result == 'success') {
        ...
        $scope.member = '登入';
        $('.member').attr('href',"http://140.119.19.39:8000/login/facebook/?next=http://140.119.19.39:8000/static/course.html?id="+id);
      }
    });
  ...
  $scope.commit = function(){
    ...
    $http.get('http://140.119.19.39:8000/score/?course_id='+$scope.courseContent.course_c_id+'&score='+$scope.scores+'&comment='+$scope.resP)
      .success(function(response){
        $scope.resP = '';
        console.log(response);
        if(response[0].result == "not login"){
          alert('請先登入！');
          location.assign('http://140.119.19.39:8000/login/facebook/?next=http://140.119.19.39:8000/static/course.html?id='+$scope.iddd );
        }
        else{
          location.reload();
        }
